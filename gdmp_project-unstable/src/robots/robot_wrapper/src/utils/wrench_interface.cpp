...
void WrenchInterface::loadParams(const std::string &filename)
{
  ...
  else this->useRobotSensor();

  // check whether to bias sensor for wrench measurements
  bool set_wrench_bias = false;
  if (parser.getParam("set_wrench_bias", set_wrench_bias) && set_wrench_bias) this->biasFTsensor();
...
void WrenchInterface::useRobotSensor()
{
