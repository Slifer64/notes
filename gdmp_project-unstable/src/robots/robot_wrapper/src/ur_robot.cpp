...
Ur_Robot::Ur_Robot(bool use_sim)
{
  ...
  init(robot_desc, base_link, tool_link, ftsensor_link, robot_ip, host_ip, reverse_port);
}

Ur_Robot::Ur_Robot(const std::string &robot_desc, const std::string &base_link, const std::string &tool_link,
  const std::string &ftsensor_link, const std::string &robot_ip, const std::string &host_ip, int reverse_port)
{
  init(robot_desc, base_link, tool_link, ftsensor_link, robot_ip, host_ip, reverse_port);
}
...
void Ur_Robot::init(const std::string &robot_desc, const std::string &base_link, const std::string &tool_link,
  const std::string &ftsensor_link, const std::string &robot_ip, const std::string &host_ip, int reverse_port)
{
    ...
    switch (mode.read()) {
      ...
      case CART_VEL_CTRL:
        if (!safety_.assertToolVelLim(cart_vel_cmd.get(), &limit_msg)) {
          ...
          // PRINT_WARNING_MSG("Tool velocity limit exceeded: " + std::to_string(arma::norm(cart_vel_cmd.get())));
          setErrMsg(limit_msg);
        ...
        {
          // if (use_svf) {
            q_dot = getInvJacobian() * cart_vel_cmd.get();
            ...
            if (!safety_.assertJointVelLim(q_dot)) {
              ...
              std::cerr << "\33[1m\33[33mJoint Velocity limit exceeded: q_dot = " << q_dot.t() << "\n\33[0m" << std::flush;
              setExternalStop(true);
