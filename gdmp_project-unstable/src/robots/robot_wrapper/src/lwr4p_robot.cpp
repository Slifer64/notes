...
LWR4p_Robot::LWR4p_Robot(bool use_sim)
{
  ...
  init(use_sim, robot_desc, base_link, tool_link, ftsensor_link, ctrl_cycle);
}

LWR4p_Robot::LWR4p_Robot(bool use_sim, const std::string &robot_desc, const std::string &base_link, const std::string &tool_link, const std::string &ftsensor_link, double ctrl_cycle)
{
  init(use_sim, robot_desc, base_link, tool_link, ftsensor_link, ctrl_cycle);
}

void LWR4p_Robot::init(bool use_sim, const std::string &robot_desc, const std::string &base_link, const std::string &tool_link, const std::string &ftsensor_link, double ctrl_cycle)
{
  ros::NodeHandle nh("~");
  ...
  robo_::KinematicChain base_sensor_chain;
  base_sensor_chain.initFromParam(robot_desc, base_link, ftsensor_link);
  arma::mat T_base_sensor = base_sensor_chain.getPose( arma::vec().zeros( base_sensor_chain.getNumOfJoints() ) );
  
  robo_::KinematicChain base_tool_chain;
  base_tool_chain.initFromParam(robot_desc, base_link, tool_link);
  arma::mat T_base_tool = base_tool_chain.getPose( arma::vec().zeros( base_tool_chain.getNumOfJoints() ) );

  arma::mat T_sensor_tool = arma::inv(T_base_sensor) * T_base_tool;

  initWrenchInterface(std::bind(&LWR4p_Robot::getTaskRotMat,this), T_sensor_tool);
...
void LWR4p_Robot::commandThread()
{
  ...
  task_vel.jpos_prev = robot->getJointsPosition();
  task_vel.vel = arma::vec().zeros(6);
  ...
  while (run_)
  {
    ...
    if (new_mode != mode.read())
    {
      switch (new_mode)
      {
        case rw_::Mode::JOINT_POS_CONTROL:
          ...
          jpos_cmd.set(robot->getJointsPosition());
          break;
        case rw_::Mode::JOINT_VEL_CONTROL:
          robot->setMode(lwr4p_::Mode::JOINT_VEL_CONTROL);
          jvel_cmd.set(arma::vec().zeros(N_JOINTS));
          break;
    ...
    switch (mode.read())
    {
      case JOINT_POS_CONTROL:
        ...
        break;
      case rw_::Mode::JOINT_VEL_CONTROL:
          robot->setJointsVelocity(jvel_cmd.get());
        break;
    ...
    arma::vec jpos_new = robot->getJointsPosition();
    task_vel.vel = getJacobian() * (jpos_new - task_vel.jpos_prev) / getCtrlCycle();
    task_vel.jpos_prev = jpos_new;
