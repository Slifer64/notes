...
public:
  ...
  std::vector<bool> getDigitalIn() const
  { return ur_driver->getDigitalIn(); }
  ...
  /* Returns the wrench measured by the robot sensor, located at the TCP (flange), expressed in the robot's base frame
   */
  ...
  /* Returns the wrench measured by the robot sensor, expressed in the robot's sensor frame, which is the TCP (flange)
   */
  ...
  void terminate() { ur_driver->terminate(); }
