...
RobotUrdf::RobotUrdf(urdf::Model &urdf_model, const std::string &base_link, const std::string &tool_link)
{
...
RobotUrdf::RobotUrdf(const std::string &robot_desc_param, const std::string &base_link, const std::string &tool_link)
{
...
void RobotUrdf::init()
{
  ...
  if (!found_base_link)
    throw std::runtime_error("[RobotUrdf Error]: Couldn't find specified base link \"" + base_link_name + "\" in the robot urdf model...\n");

  if (!found_tool_link)
    throw std::runtime_error("[RobotUrdf Error]: Couldn't find specified tool link \"" + tool_link_name + "\" in the robot urdf model...\n");
  ...
  {
    throw std::runtime_error("[RobotUrdf Error]: Failed to create kdl chain from " + base_link_name + " to " + tool_link_name + " ...\n");
  }
  else
  {
...
int RobotUrdf::getNumJoints() const
{
  return joint_names.size();
}

arma::vec RobotUrdf::getJointsPosition(const arma::mat &pose, const arma::vec &q0, bool *found_solution) const
{
...
arma::mat RobotUrdf::getTaskPose(const arma::vec &j_pos) const
{
...
arma::mat RobotUrdf::getEEJacobian(const arma::vec &j_pos) const
{
  // std::unique_lock<std::mutex> lck(robot_state_mtx);
  arma::mat R = getTaskPose(j_pos).submat(0,0,2,2);
  arma::mat Jrobot = getJacobian(j_pos);
