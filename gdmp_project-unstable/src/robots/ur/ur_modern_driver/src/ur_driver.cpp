...
void UrDriver::unpackState(char *buff, int size)
{
	...
	unpackVector(buff, size, i1, i2, joint_pos, 6);
	unpackVector(buff, size, i1, i2, joint_vel, 6);

	std::vector<double> tcp_pose(6);
	unpackVector(buff, size, i1, i2, tcp_pose, 6);
	unpackVector(buff, size, i1, i2, tcp_vel, 6);
	...
	unpackVector(buff, size, i1, i2, joint_torq, 6);
	unpackVector(buff, size, i1, i2, tcp_wrench, 6);

	digital_in.resize(8);
	std::vector<double> digial_in_temp;
	unpackVector(buff, size, i1, i2, digial_in_temp, 1);
	int digial_in_flags = digial_in_temp[0];
	for (int i=0; i<digital_in.size(); i++) digital_in[i] = digial_in_flags & (1 << i);
	// std::cerr << "digital_in: " << digital_in[0] << ", " << digital_in[1] << ", " << digital_in[2] << ", " << digital_in[3] << ", " << digital_in[4] << ", " << digital_in[5] << ", " << digital_in[6] << ", " << digital_in[7] << "\n";
}

void UrDriver::unpackVector(char *buff, int size, int &i1, int &i2, std::vector<double> &unpack_to, unsigned unpack_size)
{
	...
	if (len < unpack_size) throw std::runtime_error("[UrDriver::unpackVector]: Not enough values to unpack...\n");

	unpack_to.resize(unpack_size);
	for (int i=0; i<size; i++) iss >> unpack_to[i];
	// iss >> unpack_to[0] >> unpack_to[1] >> unpack_to[2] >> unpack_to[3] >> unpack_to[4] >> unpack_to[5];
	// for (int i=0; i<6; i++) unpack_to[i] /= MULT_JOINTSTATE_;
}
