 ...
 * IMPORTANT NOTICE!!! 
 * 
 * The TCP is set by the ur_driver_program.txt to the robot's flange!
 * This is to avoid offset issues with wrenches, poses, twists etc.
 * Therefore, in the following TCP actually refer's to the robot's flange.
 */
...
public:
  ...
  std::vector<bool> getDigitalIn() const { return digital_in; }
...
private:
  ...
  void unpackVector(char *buff, int size, int &i1, int &i2, std::vector<double> &unpack_to, unsigned unpack_size);
