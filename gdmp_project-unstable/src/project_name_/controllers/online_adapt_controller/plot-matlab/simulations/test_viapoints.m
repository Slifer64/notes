...
%% Simulate 
[Time, P_data, dP_data, ddP_data] = simulateModel(gmp, y0, g, Tf, dt, via_points);
dat{1} = struct('Time',Time, 'Pos',P_data, 'Vel',dP_data, 'Accel',ddP_data, ...
                'Color','blue', 'LineStyle','-', 'DisplayName','DMP$^{++}$');
            
dat{2} = struct('Time',Timed, 'Pos',Pd_data, 'Vel',dPd_data, 'Accel',ddPd_data, ...
...
function [Time, Y_data, dY_data, ddY_data] = simulateModel(gmp, y0, yg, Tf, dt, via_points)
    ...
    for k=1:length(via_points)
        yv_hat = model.getRefPos(via_points{k}.s);
        yv = via_points{k}.pos;
        err = norm(yv - yv_hat)
    end
