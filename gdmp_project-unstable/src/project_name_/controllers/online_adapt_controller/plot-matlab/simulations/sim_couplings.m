...
% obstacle = [obstacle {PlaneObstacle('point',[-0.14; 0.74], 'normal_angle',-75, 'color', [1 0 0])}];
...
% fig = figure; fig.Position(3:4)=[572 567];
% annotation(fig, 'textbox', 'String','Forward', 'Color',y_pl.Color, 'FontSize',18, 'LineStyle','none', 'Position',[0.2204 0.9312 0.1937 0.0577]);
% annotation(fig, 'textbox', 'String','Reverse', 'Color',y_rev_pl.Color, 'FontSize',18, 'LineStyle','none', 'Position',[0.6221 0.9312 0.1937 0.0601]);
% y_labels = {'X', 'Y'};
% for i=1:n_dof
%     ax = subplot(n_dof, 1, i);
%     ax.Box = 'on';
%     ax.FontSize = 12;
%     hold(ax, 'on');
%     frep_pl{i} = plot(nan, nan, 'LineStyle','-', 'LineWidth',2, 'Color','red', 'DisplayName','$f_{rep}$');
%     ax.XLim = [0 2*Tf];
%     ax.YLim = [-20 20];
%     plot([Tf Tf], ax.YLim, 'LineWidth',2, 'LineStyle',':', 'Color',0.7*[1 1 1], 'HandleVisibility','off');
%     if (i==n_dof), xlabel('time [$s$]', 'interpreter','latex', 'fontsize',15); end
%     if (i==1), legend({}, 'interpreter','latex', 'fontsize',17, 'Box','off', 'Position',[0.7512 0.8643 0.1446 0.0522]); end
%     ylabel([y_labels{i} ' [$N$]'], 'interpreter','latex', 'fontsize',16);
% end
...
while (true)
    ...
    Frep_data = [Frep_data f_rep];
    
    %% plot
    y_pl.XData = [y_pl.XData y(1)];
    y_pl.YData = [y_pl.YData y(2)];
%     for i=1:n_dof
%         frep_pl{i}.XData = [frep_pl{i}.XData t];
%         frep_pl{i}.YData = [frep_pl{i}.YData f_rep(i)];
%     end
...
%     for i=1:n_dof
%         frep_pl{i}.XData = [frep_pl{i}.XData t+Tf];
%         frep_pl{i}.YData = [frep_pl{i}.YData f_rep(i)];
%     end
...
% for k=1:length(ellipsoid)
%     figure;
%     F_rep = F_rep_data{k};
%     for i=1:n_dof
%         subplot(n_dof, 1, i);
%         plot(Time, F_rep(i,:), 'LineWidth',2);
%     end
% end
...
% ax = axes(); hold(ax, 'on');
% ax.Box = 'on';
% plot(Yd_data(1,:), Yd_data(2,:), 'LineStyle',':', 'LineWidth',2, 'Color','cyan', 'DisplayName','demo');
% plot(Y_data(1,:), Y_data(2,:), 'LineStyle','-', 'LineWidth',2, 'Color','blue', 'DisplayName','DMP$^{++}$');
% plot(y0d(1), y0d(2), 'LineStyle','None', 'LineWidth',3, 'MarkerSize',12, 'Marker','o', 'Color','green', 'DisplayName','$y_0$');
% plot(ygd(1), ygd(2), 'LineStyle','None', 'LineWidth',3, 'MarkerSize',12, 'Marker','x', 'Color',[1 0.6 0.6], 'DisplayName','$g_d$');
% plot(yg(1), yg(2), 'LineStyle','None', 'LineWidth',3, 'MarkerSize',12, 'Marker','x', 'Color','red', 'DisplayName','$g$');
% for i=1:length(ellipsoid)
%     plotEllipsoid(ellipsoid{i}.Sigma, ellipsoid{i}.c, ax, 'Color',ellipsoid{i}.color);
%     plotEllipsoid(ellipsoid{i}.Sigma*(1+d0), ellipsoid{i}.c, ax, 'Color', [ellipsoid{i}.color 0.4]);
% end
% lg = legend({}, 'interpreter','latex', 'fontsize',14, 'Orientation','horizontal', 'Box','off');
% lg.Position(1) = ax.Position(1) + (ax.Position(3)-lg.Position(3))/2;
% lg.Position(2) = ax.Position(2) + ax.Position(4) + 0.01;
% axis(ax, 'tight'); %, 'equal');
% ax.XLim = ax.XLim + 0.05*(ax.XLim(2) - ax.XLim(1))*[-1 1];
% ax.YLim = ax.YLim + 0.05*(ax.YLim(2) - ax.YLim(1))*[-1 1];
% plot(wall_lim*[1 1], ax.YLim, 'LineWidth',2, 'LineStyle','--', 'Color','red', 'HandleVisibility','off');
