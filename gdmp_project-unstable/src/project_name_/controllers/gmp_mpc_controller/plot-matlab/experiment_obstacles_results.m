...
%% Load GMP
gmp = GMP();
gmp_.read(gmp, '../config/obstacles_gmp_model.bin');

%% Load exec data
dat = FileIO('../data/obstacles_exec_data.bin', FileIO.in).readAll();

obst_data = dat.obstacles_data;
n_obst = obst_data(1, 1);
obst_c = obst_data(2:4, 1);
obst_Sigma = reshape(obst_data(5:13, 1), 3, 3);
...
slack_lim = dat.slack_limits;
pos_hard_lim = pos_lim + slack_lim(1)*[-1, 1];
vel_hard_lim = vel_lim + slack_lim(2)*[-1, 1];
accel_hard_lim = accel_lim + slack_lim(3)*[-1, 1];
...
obst_points = drawElipsoid(obst_Sigma, obst_c);
surf(obst_points{1}, obst_points{2}, obst_points{3}, 'LineStyle','None', 'FaceColor',[0.85 0.33 0.1], 'FaceAlpha',0.5, 'Parent',ax, 'HandleVisibility','off');
...
%     plot(Tf, ygd(i), 'LineWidth', 4, 'LineStyle','none', 'Color','magenta','Marker','x', 'MarkerSize',10, 'HandleVisibility','off', 'Parent',ax);
    ...
    % plot hard limits
    plot(ax.XLim, [pos_hard_lim(i,1) pos_hard_lim(i,1)], 'LineWidth',2, 'LineStyle','--', 'Color',[0.5 0.5 0.5], 'HandleVisibility','off', 'Parent',ax);
    plot(ax.XLim, [pos_hard_lim(i,2) pos_hard_lim(i,2)], 'LineWidth',2, 'LineStyle','--', 'Color',[0.5 0.5 0.5], 'HandleVisibility','off', 'Parent',ax);
    % labels, title ...
    ...
    % plot hard limits
    plot(ax.XLim, [vel_hard_lim(i,1) vel_hard_lim(i,1)], 'LineWidth',2, 'LineStyle','--', 'Color',[0.5 0.5 0.5], 'HandleVisibility','off', 'Parent',ax);
    plot(ax.XLim, [vel_hard_lim(i,2) vel_hard_lim(i,2)], 'LineWidth',2, 'LineStyle','--', 'Color',[0.5 0.5 0.5], 'HandleVisibility','off', 'Parent',ax);
    ...
    % plot hard limits
    plot(ax.XLim, [accel_hard_lim(i,1) accel_hard_lim(i,1)], 'LineWidth',2, 'LineStyle','--', 'Color',[0.5 0.5 0.5], 'HandleVisibility','off', 'Parent',ax);
    plot(ax.XLim, [accel_hard_lim(i,2) accel_hard_lim(i,2)], 'LineWidth',2, 'LineStyle','--', 'Color',[0.5 0.5 0.5], 'HandleVisibility','off', 'Parent',ax);
